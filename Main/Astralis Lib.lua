				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Astralis")
							if fold then
							else
								makefolder("./Astralis")
							end
							fold = isfolder("./Astralis/Themes")
							if fold then
							else
								makefolder("./Astralis/Themes")
							end
							fold = isfolder("./Astralis/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Astralis Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Astralis/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Astralis/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Astralis/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Astralis") then
						makefolder("./Astralis")
					end
					local common_string = "./Astralis/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Astralis/" .. tostring(custom_workspace or library.WorkspaceName)
						end
